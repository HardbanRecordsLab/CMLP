<?php
/**
 * HRL Amoled Premium Child — Header override
 * Jedyna zmiana względem motywu nadrzędnego: ticker (paski wiadomości)
 * pokazuje się tylko na treściach blogowych, nie na całej witrynie.
 *
...
<header class="<?php echo esc_attr( $header_classes ); ?>" role="banner" id="siteHeader">
    ...
    <?php
    // Ticker (paski wiadomości) — wyłącznie na treściach blogowych.
    $ticker_visible = get_theme_mod( 'hrl_show_ticker', true );
    if ( $ticker_visible && function_exists( 'hrl_child_is_blog_context' ) && hrl_child_is_blog_context() ) :
