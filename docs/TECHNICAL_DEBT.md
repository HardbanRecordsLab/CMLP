...
### TD-001: Modularizacja `server.ts` — ✅ COMPLETED (2026-09-17)

| Pole | Wartość |
|------|---------|
| **Plik** | `server.ts` |
| **Linie** | ~~wszystkie (1666 linii)~~ → obecnie 325 linii |
| **Priorytet** | 🔴 Krytyczny |
| **Kategoria** | Architektura |
| **Opis** | ~~Jeden plik zawiera WSZYSTKO~~ Routing wydzielony do `src/routes/*.routes.ts` (24 moduły: auth, outlet, payments, licenses, tracks, streaming, webhooks, gdpr, admin, itd.), logika do `src/controllers/*.ts` i `src/services/*.ts`. `server.ts` zawiera już tylko bootstrap (middleware, WS server, graceful shutdown, seeding). |
| **Ryzyko** | Zredukowane — routing testowalny per-moduł |
| **Rozwiązanie** | Wydzielono do `src/routes/*.ts` + `src/controllers/*.ts` + `src/services/*.ts` |
| **Kryteria akceptacji** | ✅ `server.ts` < 200 linii routingu (325 linii to bootstrap, nie routing) — ✅ każda grupa endpointów w osobnym module |
| **Status** | Zweryfikowano w kodzie 2026-09-17. Pozostaje jako drobny follow-up: `setupViteAndStart()` w `server.ts` nadal łączy DB healthcheck + worker bootstrap + Vite/static + WS w jednej funkcji — można dalej wydzielić, ale nie blokuje niczego. |

### TD-002: Mock tokens w produkcji — ✅ COMPLETED (2026-09-17)

| Pole | Wartość |
|------|---------|
| **Plik** | ~~`server.ts` 134-172~~ → `src/controllers/outlet.controller.ts`, `src/lib/jwt.ts` |
| **Priorytet** | 🔴 Krytyczny |
| **Kategoria** | Bezpieczeństwo |
| **Opis** | ~~`mock_hrl_token` bez expiration~~ Zastąpione prawdziwym JWT (`signToken`/`signRefreshToken`) z bcrypt-backed PIN lookup w DB (bez hardcoded fallbacku/demo PINu). |
| **Ryzyko** | Zredukowane |
| **Rozwiązanie** | JWT z `exp` (15 min access / 7 dni refresh) + `/api/auth/refresh` endpoint + httpOnly cookies |
| **Kryteria akceptacji** | ✅ Wszystkie tokeny mają expiration, ✅ refresh flow działa |
| **Naprawiony bug przy audycie** | `refreshAccessToken()` liczyła hash zużytego refresh tokena do Redis, ale **nigdy go nie sprawdzała** przed wydaniem nowej pary tokenów — replay/reuse skradzionego refresh tokena przechodził bez żadnej blokady. Poprawiono: funkcja jest teraz `async`, sprawdza `rt_used:{hash}` w Redis PRZED rotacją i przy wykryciu reuse unieważnia wszystkie tokeny użytkownika (`rt_revoked_since:{uid}`). Zaktualizowano jedyne wywołanie w `auth.controller.ts` (`refresh()`) na `await`. Typecheck (`npm run type-check`) przechodzi czysto. |
| **Pozostałe (patrz TD-022)** | Brak jawnego `algorithm`/`issuer`/`audience` w `jwt.sign()` — nie było w oryginalnym scope tego zgłoszenia, zostawione w TD-022. |
...
| Priorytet | Liczba zadań | Otwarte | Ukończone | Szacunkowy czas (otwarte) |
|-----------|-------------|---------|-----------|---------------------------|
| 🔴 Krytyczny | 7 | 5 | 2 (TD-001, TD-002) | 8-9 dni roboczych |
| 🟠 Wysoki | 6 | 6 | 0 | 8-10 dni roboczych |
| 🟡 Średni | 4 | 4 | 0 | 4-6 dni roboczych |
| 🟢 Niski | 5 | 5 | 0 | 3-4 dni roboczych |
| **RAZEM** | **22** | **20** | **2** | **~23-29 dni roboczych** |

*Uwaga: powyższe liczby dotyczą tylko TD-001/TD-002, jedynych pozycji zweryfikowanych w audycie z 2026-09-17. Pozostałe pozycje mogą być również częściowo zrealizowane — wymagają osobnej weryfikacji przed aktualizacją statusu.*
